...
apply_cors_headers :: proc(headers: ^map[string]string, requestHeaders: map[string]string, method: lib.HttpMethod) {
    ...
    if corsOptions == nil {
        println("ERROR: Failed to create CORS options, applying permissive CORS")
        headers["Access-Control-Allow-Origin"] = "*"
        headers["Access-Control-Allow-Methods"] = "GET,POST,PUT,DELETE,OPTIONS"
        headers["Access-Control-Allow-Headers"] = "Content-Type,Authorization,X-Requested-With"
        return
...
make_default_cors_options :: proc() -> ^lib.CorsOptions {
    ...
    if defaultCorsOptions == nil {
        println("ERROR: Failed to allocate CORS options")
        return nil
    }
    ...
    if appConfig == nil {
        println("ERROR: Failed to load app config for CORS")
